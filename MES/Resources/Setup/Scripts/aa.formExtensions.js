            ...
            aaNotifyConfigProvider.addOrUpdateNotifyConfig('aaFormExtensionsValidationErrors', {
                template:
                    '<div class="alert alert-danger aa-form-extensions-validation-errors">' +
                        ...
                        '<div class="aa-notify-close" ng-click="close(notification)">' +
                        ...
                        '<ul>' +
                            '<li ng-repeat="error in notification.validationErrorsToDisplay()">' +
                                '<span class="greyArrow"></span>' +
                                //'<span class="counter">.</span> {{ error.message }}&nbsp;' +
                                '<span class="errorText">{{ error.message }}&nbsp;</span>' +
                                '<span title="Focus Field" ng-click="notification.showField(error)" class="icon-search-new"></span>' +
                            '</li>' +
                        '</ul>' +
                        '</div>' +
                    '</div>',
            ...
            return {
                ...
                link: function (scope, element, attrs, controllers) {
                    var ngModel = controllers[0],
                        ...
                        fieldName = attrs.name || "This field";

                    if (!ngForm || !attrs.name) {
                        //only for validation with forms
                        //name is required or else angular form stuff doesn't work
                        return;
                    }

                    ensureaaFormExtensionsFieldExists(ngForm, attrs.name);
                    var field = ngForm.$aaFormExtensions[attrs.name];
                    ...
                    element.on('blur', function () {
                        ...
                        if (element[0].type == 'file') {
                            field.hadFocus = false;
                            element.removeClass('aa-had-focus');
                        }
                    ...
                    if (attrs.aaExcludeChanges === undefined) {
                        ...
                        scope.$on('$destroy', function () {
                            ...
                            delete ngForm.$aaFormExtensions[attrs.name];
                    ...
                    function calcErrorMessages() {
                        ...
                        for (var key in ngModel.$error) {
                            if (ngModel.$error[key]) {
                                ...
                                else if ($('#' + ngForm.$name + '_' + attrs.name + '_' + key).length > 0) {
                                    msg = stringFormat($('#' + ngForm.$name + '_' + attrs.name + '_' + key).text().trim());
                                    fieldErrorMessages.push(msg);
                                }
                                else if (key === 'unique') {
                                    msg = stringFormat(attrs.numberMsg || aaFormExtensions.validationMessages.unique, fieldName);
                                    fieldErrorMessages.push(msg);
                                }
            ...
            compile: function () {
                return {
                    pre: function (scope, element, attrs, thisForm) {
                        ...
                        //Call from Controller when need to Reset form.
                        scope.$on('aaFormReset', function () {
                            $reset();
                        });
                        ...
                        function $clearErrors() {
                            ...
                            $timeout(function () {
                                ...
                                angular.forEach(thisForm.$aaFormExtensions.$allValidationErrors, function (err) {
                                    ...
                                    err.field.$element.removeClass('aa-invalid-attempt');
