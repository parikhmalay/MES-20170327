...
Module.constant('datePickerConfig', {
    template: '/App_Client/common/directives/DateTimePicker/datepicker.html',
    view: 'month',
...
Module.directive('datePicker', ['datePickerConfig', function datePickerDirective(datePickerConfig) {
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.format = attrs.format || datePickerConfig.format;
            scope.min = typeof attrs.min != 'undefined' ? new Date(attrs.min.replace(/"/g, "")) : datePickerConfig.min;
            scope.max = typeof attrs.max != 'undefined' ? new Date(attrs.max.replace(/"/g, "")) : datePickerConfig.max;

            var newmin;
            if (scope.view == "date")
                newmin = new Date(scope.min.getFullYear(), scope.min.getMonth(), scope.min.getDate());
            else if (scope.view == "hours")
                newmin = new Date(scope.min.getFullYear(), scope.min.getMonth(), scope.min.getDate(), scope.min.getHours(), scope.min.getMinutes());
            scope.min = newmin;
            ...
            scope.isDisabled = function (date) {
                if (scope.view == 'date') {
                    if (scope.min.getTime() <= date.getTime() && scope.max.getTime() >= date.getTime())
                        return false;
                    else
                        return true;
                }
