...
.provider('unsavedWarningsConfig', function () {
    ...
    var navigateMessage = 'You have unsaved changes.';// Are you sure you want to leave this page?';
    var reloadMessage = 'You have unsaved changes.';// Are you sure you want to leave this page?';
            ...
            angular.forEach(eventsToWatchFor, function (aEvent) {
                ...
                var removeFn = $rootScope.$on(aEvent, function (event, next, current) {
                    ...
                    if (!allFormsClean() && !alertOnce) {
                        ...
                        if (aEvent == 'ModalSliderDismiss' && !allFormsClean()) {
                            if (!confirm(messages.navigate)) {
                                unsavedWarningsConfig.log("user wants to cancel leaving");
                                confirmresult = false;
                                event.preventDefault(); // user clicks cancel, wants to stay on page 
                    ...
                    if (aEvent == 'ModalSliderDismiss') {
                        $rootScope.$broadcast('ModalSliderDismissConfirm', { confirmresult: confirmresult, modalInstance: next.modalInstance, reason: next.reason });
                    }
                });
                alertOnce = false;
