app.controller('SupplierQuoteCtrl', ['$rootScope', '$scope', 'common', 'SupplierQuoteSvc', '$timeout', '$filter', 'LookupSvc', '$routeParams', '$window',
    function ($rootScope, $scope, common, SupplierQuoteSvc, $timeout, $filter, LookupSvc, $routeParams, $window) {
        ...
        if (!IsUndefinedNullOrEmpty($routeParams.RFQId) && !IsUndefinedNullOrEmpty($routeParams.SupplierId) && $routeParams.SupplierId != 'null' && parseInt($routeParams.SupplierId) != 0) {
        ...
        $scope.RFQSupplierPartQuoteListHistory = {};
        //initialize variables
        $scope.Init = function () {
            advanceSearch.open();
            advanceSearch.funEscape();
            $scope.IsCustomerValid = true;
            $scope.IsRFQValid = true;
            $scope.IsSupplierValid = true;
            $scope.CurrentTab = 'PartDetail';
            $scope.CurrentTabHistory = 'PartDetailHistory';
            $scope.EnterQuote = true;
            $scope.NoQuote = false;
            $scope.IsForm = false;
            $scope.RFQDetail = {};
            $scope.Paging = {};
            $scope.SearchCriteria = { CustomerId: 0, RFQId: '', SupplierId: 0, IsQuoteTypeDQ: false };

            if ($scope.IsCalledFromRFQ) {
                $scope.SearchCriteria = { CustomerId: 0, RFQId: $routeParams.RFQId, SupplierId: parseInt($routeParams.SupplierId), IsQuoteTypeDQ: false };
            }
            $scope.Paging.Criteria = $scope.SearchCriteria;
            $scope.SetLooksupDataWhenCalledFromRFQ();
            if ($scope.IsCalledFromRFQ) {
                setTimeout(function () {
                    $scope.SearchCriteria = { CustomerId: 0, RFQId: $routeParams.RFQId, SupplierId: parseInt($routeParams.SupplierId), IsQuoteTypeDQ: false };
                    $scope.Paging.Criteria = $scope.SearchCriteria;
        ...
        $scope.setRequiredMessage = function () {
            $scope.IsCustomerValid = true;
            $scope.IsRFQValid = true;
            $scope.IsSupplierValid = true;
            if ($scope.SearchCriteria.CustomerId == 0) {
                $scope.IsCustomerValid = false;
                return;
            }
            if (IsUndefinedNullOrEmpty($scope.SearchCriteria.RFQId)) {
                $scope.IsRFQValid = false;
                return;
            }
            if ($scope.SearchCriteria.SupplierId == 0) {
                $scope.IsSupplierValid = false;
                return;
            }
        };
            ...
            SupplierQuoteSvc.GetSupplierQuoteList($scope.Paging).then(
                 function (response) {
                     if (response.data.StatusCode == 200) {                  //ShowMessage(common, response.data)
                         ...
                         if (!Isundefinedornull($scope.SupplierForSupplierQuoteList) && $scope.SupplierForSupplierQuoteList.length > 0) {
                             var supName = $filter('filter')($scope.SupplierForSupplierQuoteList, function (rw) { return rw.Id == $scope.SearchCriteria.SupplierId })[0];
                             $scope.RFQDetail.SupplierName = IsUndefinedNullOrEmpty(supName) ? "" : supName.Name;
                         }
                         else
                             $scope.RFQDetail.SupplierName = "";
        ...
        $scope.getRFQDetails = function () {
            if (IsUndefinedNullOrEmpty($scope.SearchCriteria.RFQId)) {
                $scope.RFQDetail = {};
                return;
            }
            SupplierQuoteSvc.getRFQDetails($scope.Paging).then(
                     function (response) {
                         if (response.data.StatusCode == 200) {
                             $scope.RFQDetail = response.data.Result;
                             $scope.SearchCriteria.CustomerId = $scope.RFQDetail.CustomerId;
                         }
                     },
                    function (error) {
                        common.usSpinnerService.stop('spnRFQSupplierPartQuote');
        ...
        $scope.SetLooksupData = function () {
            $scope.LookUps = [
              {
                  "Name": "CustomerForSupplierQuote", "Parameters": { "PartNumber": $scope.SearchCriteria.PartNumber }
              },
              {
                  "Name": "RFQForSupplierQuote", "Parameters": { "PartNumber": $scope.SearchCriteria.PartNumber, "CustomerId": $scope.SearchCriteria.CustomerId }
              },
              {
        ...
        $scope.SetLooksupDataWhenCalledFromRFQ = function () {
            $scope.LookUps = [
              {
                  "Name": "CustomerForSupplierQuote", "Parameters": { "PartNumber": $scope.SearchCriteria.PartNumber }
              },
        ...
        $scope.isActiveHistory = function (route) {
            return ($scope.CurrentTabHistory == route);
        };
        ...
        $scope.OnCustomerChange = function () {
            ...
            if ((!IsUndefinedNullOrEmpty($scope.SearchCriteria.CustomerId) && parseInt($scope.SearchCriteria.CustomerId) > 0) || !IsUndefinedNullOrEmpty($scope.SearchCriteria.PartNumber)) {
                $scope.LookUps = [
                        {
                            "Name": "RFQForSupplierQuote", "Parameters": { "PartNumber": $scope.SearchCriteria.PartNumber, "CustomerId": IsUndefinedNullOrEmpty($scope.SearchCriteria.CustomerId) ? 0 : $scope.SearchCriteria.CustomerId }
                        },
                        {
                            "Name": "SupplierForSupplierQuote", "Parameters": { "RFQId": $scope.SearchCriteria.RFQId }
                        }
                ...
                $scope.RFQForSupplierQuoteList = [];
                $scope.SupplierForSupplierQuoteList = [];
            }
        ...
        $scope.OnRFQChange = function () {
            ...
            $scope.getRFQDetails();
            setTimeout(function () {
                if (!Isundefinedornull($scope.RFQForSupplierQuoteList) && $scope.RFQForSupplierQuoteList.length > 0) {
                    var rowObject = $filter('filter')($scope.RFQForSupplierQuoteList, function (rw) { return rw.Id == $scope.SearchCriteria.RFQId })[0];
                    $scope.SearchCriteria.CustomerId = IsUndefinedNullOrEmpty(rowObject) ? $scope.SearchCriteria.CustomerId : rowObject.ParentId;
                }
                $scope.LookUps = [
                    {
                        "Name": "RFQForSupplierQuote", "Parameters": { "PartNumber": $scope.SearchCriteria.PartNumber, "CustomerId": $scope.SearchCriteria.CustomerId }
                ...
                $scope.getLookupData();
                if ($scope.IsCalledFromRFQ)
                    setTimeout($scope.OnSupplierChange(), 300);
            }, 600);
        }
        $scope.OnSupplierChange = function () {
            $scope.SearchCriteria.SupplierId = IsUndefinedNullOrEmpty($scope.SearchCriteria.SupplierId) ? 0 : $scope.SearchCriteria.SupplierId;
            $scope.SearchCriteria.CustomerId = IsUndefinedNullOrEmpty($scope.SearchCriteria.CustomerId) ? 0 : $scope.SearchCriteria.CustomerId;
            $scope.Paging.Criteria = $scope.SearchCriteria;
            $scope.setParameters();
            $scope.GetRFQSupplierPartQuoteList();
        }
        $scope.$watch("SearchCriteria.SupplierId", function (newVal, OldVal) {
            if (!IsUndefinedNullOrEmpty(newVal) && newVal != 0) {
                //var supName = $filter('filter')($scope.SupplierForSupplierQuoteList, function (rw) { return rw.Id == newVal })[0];
                //$scope.RFQDetail.SupplierName = IsUndefinedNullOrEmpty(supName) ? "" : supName.Name;
                $scope.IsForm = true;
            }
            else
        ...
        $scope.$watch("SearchCriteria.RFQId", function (newVal, OldVal) {
            if (IsUndefinedNullOrEmpty(newVal))
                $scope.RFQDetail = {};
        });
        $scope.$watch("EnterQuote", function (newVal, OldVal) {
            if ($scope.EnterQuote == false) {
                $scope.setRequiredMessage();
            }
        });

        var searchTimeoutDelay;
        $scope.searchAndSetSearchDropdownsData = function () {
            if (searchTimeoutDelay) {
                $timeout.cancel(searchTimeoutDelay);
            }
            searchTimeoutDelay = $timeout(function () {
                $scope.SetSearchDropdownsData();
            }, 1000);
        };
        $scope.SetSearchDropdownsData = function () {
            if (!IsUndefinedNullOrEmpty($scope.SearchCriteria.PartNumber))
                $scope.SetLooksupData();
            else {
                $scope.RFQForSupplierQuoteList = [];
                $scope.SetLooksupDataWhenCalledFromRFQ();
                ...
                if (!IsObjectEmpty($scope.RFQSupplierPartQuoteList)) {
                   ...
                   function (response) {
                       ...
                       if (ShowMessage(common, response.data)) {
                           ...
                           if (closeForm)
                               $window.location.href = "/RFQ/RFQ/RFQ#/AddEdit/" + $scope.SearchCriteria.RFQId + "/0";
        ...
        $scope.BackToRFQ = function () {
            $window.location.href = "/RFQ/RFQ/RFQ#/AddEdit/" + $routeParams.RFQId + "/0";
        };
